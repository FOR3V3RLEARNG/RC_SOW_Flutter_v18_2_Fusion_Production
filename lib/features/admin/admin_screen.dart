        ...
        body: const Center(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Icon(Icons.lock_outline, size: 54, color: RcColors.brand),
                SizedBox(height: 14),
                Text(
                  'Admin access only',
                ...
                SizedBox(height: 8),
                Text(
                  'This area is restricted to the Admin role.',
            ...
            children: [
              Wrap(
                ...
                children: [
                  Chip(
                    ...
                    label: const Text('Admin Only'),
                  ),
                  Chip(
                    label: Text(
                      '${list.length} pending registration${list.length == 1 ? '' : 's'}',
                    ),
                  ),
                ],
              ),
              ...
              if (snap.hasError)
                Card(
                  child: Padding(
                    padding: const EdgeInsets.all(16),
                    child: Text('Could not load requests: ${snap.error}'),
                  ),
                ),
              if (snap.connectionState != ConnectionState.waiting &&
                  list.isEmpty)
                  ...
                  children: list
                      .map(
                        (r) => ListTile(
                          ...
                          subtitle: Text(
                            '${r['requested_role'] ?? 'Role pending'} • ${r['requested_parish'] ?? 'Parish pending'}',
                          ),
                          trailing: PopupMenuButton<String>(
                            onSelected: (action) =>
                                _act(action, '${r['user_id']}'),
                            itemBuilder: (_) => const [
                              PopupMenuItem(
                                value: 'approve',
                                child: Text('Approve'),
