...
class _PremiumSplashState extends State<PremiumSplash>
  ...
  void initState() {
    ...
    controller = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: widget.reduceMotion ? 1 : 1550),
    )..forward();
    timer = Timer(
      Duration(milliseconds: widget.reduceMotion ? 250 : 1750),
      widget.onComplete,
    );
  }
                  ...
                  Container(
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        CustomPaint(
                          painter: RoofRepairPainter(controller.value),
                        ),
                      ],
                    ),
                  ),
...
class RoofRepairPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void beam(Offset a, Offset b, double p) => canvas.drawLine(
      a,
      Offset.lerp(a, b, Curves.easeOutCubic.transform(p))!,
      paint,
    );
    ...
    beam(
      Offset(ridge.dx - 26, ridge.dy),
      Offset(ridge.dx + 26, ridge.dy),
      stage(.52, .68),
    );
    beam(
      Offset(left.dx + 25, left.dy),
      Offset.lerp(left, ridge, .55)!,
      stage(.64, .8),
    );
    beam(
      Offset(right.dx - 25, right.dy),
      Offset.lerp(right, ridge, .55)!,
      stage(.72, .9),
    );
  ...
  @override
  bool shouldRepaint(covariant RoofRepairPainter oldDelegate) =>
      oldDelegate.t != t;
}
