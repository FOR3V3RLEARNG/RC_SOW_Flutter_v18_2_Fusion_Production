class UserProfile {
  ...
  bool get canManageTemplates =>
      isAdmin || hasPrivilege('manageFolders') || canViewAllParishes;
...
class ActiveUserRecord {
  ...
  factory ActiveUserRecord.fromMap(Map<String, dynamic> map) =>
      ActiveUserRecord(
        userId: '${map['user_id'] ?? ''}',
        email: '${map['email'] ?? ''}',
        ...
        role: '${map['role'] ?? ''}',
        parish: '${map['parish'] ?? ''}',
        online: map['active'] == true,
        lastSeen: DateTime.tryParse(
          '${map['lastSeen'] ?? map['last_seen'] ?? ''}',
        ),
      );
}
...
class ManagedUserRecord {
  ...
  final Map<String, dynamic> privileges;
  String get displayName => fullName.trim().isEmpty ? email : fullName;
  factory ManagedUserRecord.fromMap(Map<String, dynamic> map) =>
      ManagedUserRecord(
        userId: '${map['user_id'] ?? ''}',
        email: '${map['email'] ?? ''}',
        fullName: '${map['full_name'] ?? ''}',
...
class MessageRecord {
  ...
  factory MessageRecord.fromEvent(Map<String, dynamic> row, String email) {
    ...
    return MessageRecord(
      ...
      sender:
          '${item['senderName'] ?? (senderEmail.trim().isEmpty ? 'RC SOW' : senderEmail)}',
...
class RoofMeasurements {
  ...
  double get roofAreaSqFt =>
      2 * (lengthFt + 2 * overhangFt) * (rafterLengthFt + overhangFt);
}

class RoofWallSegment {
  const RoofWallSegment({
    required this.label,
    required this.lengthFt,
    this.hasDrain = false,
  });
