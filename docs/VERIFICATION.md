...
Release 20.0.0 was prepared on 4 September 2026. The previous 19.1.0 source passed the GitHub Green Gate on Flutter 3.47.2; this expanded release must run its own gate after publication.

## Static preflight completed here

- 34 Dart source/test files passed delimiter and string/comment closure checks
- All 60 declared routes have an explicit screen case
...
Flutter is not installed in this scratch runtime, so no new analyzer/build success is claimed before the 20.0.0 GitHub Actions run completes. The full sequence is defined in `.github/workflows/green_gate.yml` and `tool/bootstrap_and_verify.sh`.
